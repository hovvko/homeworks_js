...
let parseTitle = JSON.parse(title)

let h1Header = document.createElement("h1");
document.body.appendChild(h1Header);
h1Header.innerText = "Post of chosen user:";

for (let arrPostInfoElement of arrPostInfo) {
    let userId = {/** @param arrPostInfoElement.userId **/};
    let postId = {/** @param arrPostInfoElement.postId **/};

    if (arrPostInfoElement.title === parseTitle) {
        ...
        fetch(`https://jsonplaceholder.typicode.com/posts/${arrPostInfoElement.id}/comments`).then(response => {
            response.json().then(json => {
                ...
                h2Comment.innerText = `Comments:`;

                let divMainComment = document.createElement("div");
                document.body.appendChild(divMainComment);
                divMainComment.classList.add("mDivStyle");

                for (let jsonElement of json) {
                    let divComment = document.createElement("div");
                    divMainComment.appendChild(divComment);
                    divComment.classList.add("styleD");
                    divComment.style.width = "24%";
